...
function write_yaml_variables(created_file, rule_yaml_table)
...
function write_function(created_file, rule_yaml_table, modulename, modulepath)
    local priority = rule_yaml_table.priority or 1
    if priority > 100 then priority = 100 end
    ...
    if string.match( name, "rules") then
        package.preload[name] = function(modulename)
            ...
            if file then
                ...
                local rule_yaml_table = get_and_decode_yaml(modulepath)
                write_yaml_variables(created_file, rule_yaml_table)
                write_function(created_file, rule_yaml_table, modulename, modulepath)
