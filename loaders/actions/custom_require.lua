    ...
    if string.match( name, "actions") then
      package.preload[name] = function(modulename)
        ...
        if file then
          ...
          actions_loader.write_events(created_file, header, modulepath)
          actions_loader.set_priority(created_file, header, modulepath)
          actions_loader.write_input_parameters(created_file, header, modulepath)
          actions_loader.write_function(created_file, header, modulepath)
          actions_loader.write_return(created_file, header, modulepath)
